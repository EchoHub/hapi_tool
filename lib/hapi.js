...
function HapiUtil(options) {
    this.name = options.name || ""
    this.path = options.path || ""
    this.apis = options.filePath ? require(options.filePath) : ""
    ...
    this.anazApis = function anazApis(api) {
        ...
        if (options && options.forwardUrl) {
            headers["forwardUrl"] = options.forwardUrl
        }
...
HapiUtil.prototype = {
    build: async function (type) {
        try {
            ...
            return pump([
                gulp.src(`${root}/src/${type}/_template/*.*`),
                gulpif(!hasExist,
                    gulp
                        .dest(destPath)
                        .on("end", function () {
